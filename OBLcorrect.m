function [positions, opposite_positions] = opposition_based_learning(positions, LB, UB)
    [nParticles, nDimensions] = size(positions);
    opposite_positions = zeros(nParticles, nDimensions);

    for i = 1:nParticles
        for j = 1:nDimensions
            opposite_positions(i, j) = LB(1, j) + UB(1, j) - positions(i, j);
        end

        fitness1 = fitnessFunction(positions(i, :));
        fitness2 = fitnessFunction(opposite_positions(i, :));
...
function [bestFitnessOverTime] = particleSwarmOptimization(nParticles, maxIterations, nDimensions, LB, UB)
    ...
    initial_positions = LB + (UB - LB) .* rand(nParticles, nDimensions);
    positions=opposition_based_learning(initial_positions, LB, UB);
